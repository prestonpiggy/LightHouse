...
public class OceanAdvanced : MonoBehaviour
{
  ...
  static Wave[] waves =
  {
    new Wave(45, 1.0f, 1.4f, 0.9f, new Vector2(1.0f,  0.2f)),
    new Wave(30, 1.2f, 0.8f, 0.5f, new Vector2(1.0f,  3.0f)),
    new Wave(15, 3.5f, 0.4f, 0.8f, new Vector2(2.0f,  15.0f)),//was 4
    new Wave(15, 2.0f, 0.4f, 0.4f, new Vector2(-1.0f, 0.0f)),
    new Wave(5, 3.0f, 0.05f, 0.9f,new Vector2(-1.0f, 1.2f))
  };
  ...
  static public float GetWaterHeight(Vector3 p)
  {
    float height = 0;
    for (int i = 0; i < NB_WAVE; i++)
      height += waves[i].amplitude * Mathf.Sin(Vector2.Dot(waves[i].direction, new Vector2(p.x, p.z)) * waves[i].frequency + Time.time * waves[i].phase)-0.7f;
    return height;
  }
}
